...
setInterval(function() {
    db.each('SELECT * FROM (SELECT * FROM (SELECT * FROM milk WHERE dateonly = DATE("now", "-1 day"))AS day ORDER BY date DESC LIMIT 6) AS date ORDER BY date LIMIT 1', function(err, row) {
        if (err) {
            console.log('Oh no!' + err.message)
            return
        }
...
setInterval(function() {
    db.each('SELECT * FROM (SELECT * FROM (SELECT * FROM milk WHERE dateonly = DATE("now", "-1 day"))AS day ORDER BY date DESC LIMIT 7) AS date ORDER BY date LIMIT 1', function(err, row) {
        if (err) {
            console.log('Oh no!' + err.message)
            return
        }
